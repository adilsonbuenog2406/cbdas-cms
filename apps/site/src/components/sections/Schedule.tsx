      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6">
        ...
        <div className="relative mx-auto max-w-[72rem]">
          ...
          <div className="mb-8 flex justify-center md:mb-9">
            <div data-program-tabs className="inline-flex rounded-full bg-white p-[3px] shadow-sm">
              {scheduleDays.map((day, index) => (
                <button
                  key={day.id}
                  type="button"
                  data-program-tab={`day-${index + 1}`}
          ...
          <div className="mx-auto min-h-[440px] max-w-[42rem] md:min-h-[480px]">
            {scheduleDays.map((day, index) => (
              <motion.div
                key={day.id}
                data-program-day={`day-${index + 1}`}
                hidden={activeDay !== index}
                initial={{ opacity: 0, y: 14 }}
                animate={activeDay === index ? { opacity: 1, y: 0 } : { opacity: 0, y: 14 }}
